...
mod TrivexAction {
    ...
    struct Storage {
        ...
        strategy_addresses: LegacyMap<u256, UserAddress>,
        strategy_addresses_len: u256,
    }
    ...
    impl TrivexActionImpl of super::ITrivexAction<ContractState> {
        ...
        fn run_strategy(ref self: ContractState, strategy: felt252, amount: Amount, token_address: TokenAddress) {
            ...
            if count == 0 {
                let current_user_count = self.strategy_addresses_len.read();
                self.strategy_addresses.write(current_user_count, caller);
                self.strategy_addresses_len.write(current_user_count + 1);
            };
        ...
        fn get_all_histories(self: @ContractState) -> Array<history_entry> {
            ...
            let total_users = self.strategy_addresses_len.read();
            let mut u = 0;

            while u < total_users {
                let user = self.strategy_addresses.read(u);
                let count = self.user_history_counts.read(user);
                let mut i = 0;
        ...
        fn get_all_user_addresses_strategy(self: @ContractState) -> Array<UserAddress> {
            let length = self.strategy_addresses_len.read();
            let mut addresses = ArrayTrait::new();
            let mut i = 0_u256;
